...
| 분류 | 사용 기술 | 상세 |
|---|---|---|
| 언어 | ![Swift](https://img.shields.io/badge/Swift-F54A2A?style=for-the-badge&logo=swift&logoColor=white) | Swift |
...
| 평가 기준 | 대응 방식 |
|---|---|
| 효용성 | 브라우저 북마크, 카카오톡, 노션 등에 분산된 링크를 한 곳에서 저장하고 소비할 수 있어 실제 사용 불편 해결 |
| 완결성 | 링크 저장 → 카드 뷰어 소비 → 읽음 처리 → 카테고리 관리 → Firebase 동기화까지 의도한 흐름 구현 |
| 직관성 | 스와이프 제스처, 미읽음 필터, 최신순/오래된순 정렬 토글 등 iOS 사용자에게 익숙한 인터랙션으로 구성 |
| 라벨링 | "링크 저장", "카드로 읽기", "읽음 표시" 등 기능을 직관적으로 설명하는 한국어 표현 사용 |
| 시각디자인 | 카드 뉴스 형식의 UI, 카테고리별 색상 구분, 커버/핵심 메시지/인사이트 3단계 카드 레이아웃으로 가독성 향상 |
| 학습용이성 | 3단계 온보딩(환영 → 관심 분야 선택 → 사용법 안내)으로 처음 사용자도 흐름을 쉽게 파악 가능 |
| 피드백 | 저장된 링크 수 표시, 읽음 상태 자동 추적, 카드 마지막 장 도달 시 읽음 처리 등 현재 상태를 명확히 제공 |
| 오류 정정 | 스와이프 삭제 시 확인 절차를 두어 실수로 인한 삭제를 방지, 카테고리 미선택 시 기본값으로 자동 설정 |
| 정보성 | README에 프로젝트 배경/기능/기술 스택/구조도를 명시, 시연 영상을 통해 시각적으로 확인 |
